...
fn main() {
    ...
    cortex_m::interrupt::free(|cs| {
        ...
        init_i2c(rcc, gpiob, i2c);
        if let Err(_) = write_i2c(vl53l0x_address, &[0xC0], i2c) {
            led_error.high();
            loop {}
        }
        let mut buffer = [0u8; 1];
        if let Err(_) = read_i2c(vl53l0x_address, &mut buffer, i2c) {
            led_error.high();
            loop {}
        }
...
///
/// In addition, this function does all its work by accessing the registers
/// directly, which might interfere with other modules that use the same
/// registers.
fn init_i2c(rcc: &RCC, gpiob: &GPIOB, i2c: &I2C1) {
...
fn write_i2c(address: u8, data: &[u8], i2c: &I2C1) -> Result<(), WriteError> {
...
///
/// Reads data from the I2C bus until the buffer has been filled. Blocks until
...
fn read_i2c(address: u8, buffer: &mut [u8], i2c: &I2C1)
    -> Result<(), ReadError>
{
